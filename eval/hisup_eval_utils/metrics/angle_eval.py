...
def compute_polygon_contour_measures(pred_polygons: list, gt_polygons: list, sampling_spacing: float, min_precision: float, max_stretch: float, metric_name: str="cosine", progressbar=False):
    ...
    # filtered_pred_polygons = [pred_polygon for pred_polygon in pred_polygons.geoms if min_precision < pred_polygon.intersection(gt_polygons).area / pred_polygon.area if pred_polygon.area > 0.]
    filtered_pred_polygons = []
    for pred_polygon in pred_polygons.geoms:
        if pred_polygon.area > 0.:
            if min_precision < pred_polygon.intersection(gt_polygons).area / pred_polygon.area:
                filtered_pred_polygons.append(pred_polygon)
...
def fix_polygons(polygons, buffer=0.0):
    ...
    if polygons_geom.geom_type == "MultiPolygon":
        for poly in polygons_geom.geoms:  # updated polygons_geom -> polygons_geoms.geoms Refer: https://shapely.readthedocs.io/en/stable/migration.html#multi-part-geometries-will-no-longer-be-sequences-length-iterable-indexable
