# Borrowed from https://github.com/SarahwXU/HiSup/blob/main/tools/inria_to_coco.py
...
if __name__ == '__main__':
    ...
    output_data_train = {
        'info': {'district': 'Christchurch, NZ', 'description': 'WHU building footprints', 'contributor': 'whu'},
    ...
    for g_id, label in enumerate(tqdm(input_label)):
        ...
        if split == "test":
            label_name = label_info[0] + label_info[1] + label_info[2] + label_info[3]
        else:
        ...
        for pid, pa in enumerate(patch_list):
            ...
            for p_im, p_gt in zip(p_ims, p_gts):
                if np.sum(p_gt > 0) > 5:
                        ...
                        if p_area > 0:
                            ...
                            if p_bbox[2] > 5 and p_bbox[3] > 5:
                                ...
                                for part_poly in coor_list:
                                    ...
                                    out_poly = part_poly + np.array([x_ul, y_ul])
                                    out_poly = Polygon(out_poly)
                                    tile_polygons.append(out_poly)
                                anno_info = {
                                    'id': ob_id,
                                    'image_id': im_id,
                                    'segmentation': p_seg,
                    ...
                    ob_id += 1
                # get patch info
                p_name = label_name + '-' + str(im_id) + '.tif'
                patch_info = {'id': im_id, 'file_name': p_name, 'width': patch_size, 'height': patch_size}
