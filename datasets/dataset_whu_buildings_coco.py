...
class WHUBuildingsCocoDataset(Dataset):
    def __init__(self, dataset_dir, transform=None, tokenizer=None, shuffle_tokens=False):
        ...
        # image_ids = [int(im.split('-')[-1].split('.')[0]) for im in self.images]
        image_ids = self.coco.getImgIds(catIds=self.coco.getCatIds())
        if "train" in dataset_dir:
            # remove images with more than 144 vertices from training.
            self.image_ids = [im for im in image_ids if int(im) not in [16608, 36020, 36021]]
        else:
...
class WHUBuildingsCocoDataset_val(Dataset):
...
def whu_buildings_collate_fn(batch, max_len, pad_idx):
    """
    if max_len:
...
class WHUBuildingsCocoDatasetTest(Dataset):
