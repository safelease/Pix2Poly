    ...
    def __getitem__(self, index):
        ...
        for ins in annotations:
            ...
            for i, segm in enumerate(segmentations):
                ...
                segm[:, 0] = np.clip(segm[:, 0], 0, img['width'] - 1e-4)
                segm[:, 1] = np.clip(segm[:, 1], 0, img['height'] - 1e-4)
                points = segm[:-1]
...
class CrowdAIDataset_val(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_idx = int(im_name.split('.')[0])
        # img_idx = self.image_ids[index]
        img = self.coco.loadImgs(img_idx)[0]
        img_path = osp.join(self.image_dir, img["file_name"])
        ann_ids = self.coco.getAnnIds(imgIds=img['id'])
        annotations = self.coco.loadAnns(ann_ids)  # annotations of all instances in an image.
        ...
        image_idx = torch.tensor([img_idx])
...
class CrowdAIDatasetTest(Dataset):
